...
{#if working}
	...
	<div
		class="flex flex-col gap-1 rounded-[var(--r-lg)] border border-[var(--live-line)] bg-[var(--live-bg)] px-4 py-3 backdrop-blur-md"
		role="status"
		aria-label="{who} is working"
	>
		<div class="flex items-center gap-2">
			<span class="inline-block h-1.5 w-1.5 animate-pulse rounded-[var(--r-pill)] bg-[var(--live)]"
			></span>
			<span class="text-[12px] font-medium tracking-wide text-[var(--t1)]">{who} is on it</span>
		</div>
		{#if stepLine}
			<div class="text-[11px] text-[var(--t3)]">{stepLine}</div>
		{/if}
	</div>
{:else if succeeded}
	...
	<div
		class="inline-flex items-center gap-1.5 rounded-[var(--r-pill)] border border-[var(--green-line)] bg-[var(--green-bg)] px-3 py-1 text-[11px] text-[var(--t2)]"
	>
...
{:else}
	...
	<div
		class="flex items-center gap-2 rounded-[var(--r-lg)] border border-[var(--red-line)] bg-[var(--red-bg)] px-4 py-2 text-[12px] text-[var(--t2)]"
	>
		<span
			>{status === 'aborted'
