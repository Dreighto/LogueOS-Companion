...
test('Phase 6 iPhone WebKit probe', async ({ page }, testInfo) => {
    ...
    const viewportBefore = await page.evaluate(() => ({ w: window.innerWidth, h: window.innerHeight }));
    ...
    const viewportAfter = await page.evaluate(() => ({ w: window.innerWidth, h: window.innerHeight }));
